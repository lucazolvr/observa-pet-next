...
export type CreatePetAdminResult =
  | { post: FeedPost; error?: never }
  | { error: string; post?: never }

export async function createPetAdmin(formData: FormData): Promise<CreatePetAdminResult> {
  ...
  const { data: post, error: postError } = await supabase
    ...
    .select(`
      id, type, caption, neighborhood, location_text, created_at, pet_id,
      pet:pets!pet_id(id, name, species, status, overview, traits, neighborhood),
  ...
  return { post: post as unknown as FeedPost }
}
